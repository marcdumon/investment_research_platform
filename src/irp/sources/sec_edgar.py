...
def _parse_period(period: str) -> tuple[int, str, int | None]:
    ...
    if period.endswith("A") or period.endswith("Q4"):
        # Q4 is not a separate filing — it's the annual report
        return year, "A", None
    q = int(period[-1])
    return year, "Q", q


def _pick_filing(filings: dict, year: int, kind: str, quarter: int | None) -> tuple[str, str, str]:
    candidates = []
    for form, date, acc, doc in zip(
        ...
        y = int(date[:4])
        if kind == "A" and form == "10-K" and y == year:
            candidates.append((date, acc, doc))
        if kind == "Q" and form == "10-Q" and y == year:
            if quarter is None:
                candidates.append((date, acc, doc))
            else:
                m = int(date[5:7])
...
def sec_filing_url(ticker: str, period: str) -> str:
    ...
    _, acc, doc = _pick_filing(filings, year, kind, quarter)
