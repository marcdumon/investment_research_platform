...
def walk_forward_classifier(
    ...
    if target not in df.columns:
        raise ValueError(f'no "{target}" column — build a quantile/binary label on /features.')
    ...
    if not preds:
        return ClassifierResult(pd.DataFrame(), [], np.nan, pd.Series(dtype=float),
                                np.empty((0, 0)), feature_cols)
    ...
    res = ClassifierResult(
        predictions=pred_df, classes=classes, accuracy=acc,
        accuracy_by_date=acc_by_date, confusion=cm, feature_cols=feature_cols,
    )
    if has_ret:
        ...
        res.icir = float(res.ic_series.mean() / res.ic_series.std()) if res.ic_series.std() else np.nan
    return res
...
def summary(res: ClassifierResult) -> pd.DataFrame:
    base = res.predictions['label'].value_counts(normalize=True).max() if len(res.predictions) else np.nan
    out = pd.DataFrame([{
        'accuracy': res.accuracy, 'baseline_acc (majority)': base,
        'n_classes': len(res.classes), 'n_dates': len(res.accuracy_by_date),
        'n_preds': len(res.predictions),
        'score_IC': res.mean_ic, 'score_ICIR': res.icir,
    }])
...
def plot_confusion(res: ClassifierResult, template: str = 'plotly_dark') -> go.Figure:
    ...
    fig = go.Figure(go.Heatmap(
        z=norm, x=labels, y=labels, colorscale='Blues', zmin=0, zmax=1,
        text=cm, texttemplate='%{text}', hovertemplate='true %{y} → pred %{x}<extra></extra>'))
    fig.update_layout(bl._layout(template, title='Confusion (row-normalized)',
...
def plot_accuracy(res: ClassifierResult, template: str = 'plotly_dark') -> go.Figure:
    ...
    fig.add_hline(y=res.accuracy, line_dash='dash', line_color='#fff',
                  annotation_text=f'mean {res.accuracy:.3f}')
    fig.update_layout(bl._layout(template, title='Out-of-sample accuracy per date',
...
def plot_quintiles(res: ClassifierResult, template: str = 'plotly_dark') -> go.Figure:
    if res.quintile_cumret is None or res.quintile_cumret.empty:
        return go.Figure(layout=bl._layout(
