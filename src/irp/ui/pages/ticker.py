...
def _period_radio(prefix: str) -> html.Div:
    return html.Div(className='stmt-controls', children=[
        dcc.RadioItems(
            id=f'{prefix}-period',
            options=[
...
layout = html.Div(className='ticker-page', children=[
    html.Div(className='ticker-search-bar', children=[
        dcc.Dropdown(
            id='ticker-select',
        ...
        dcc.Dropdown(id='filter-market', placeholder='Market', clearable=True, className='filter-dropdown'),
        dcc.Dropdown(id='filter-sector', placeholder='Sector', clearable=True, className='filter-dropdown'),
        dcc.Dropdown(id='filter-industry', placeholder='Industry', clearable=True, className='filter-dropdown'),
    ]),
    ...
    html.Div(id='ticker-content', style=_HIDE, children=[
        html.Div(id='ticker-header', className='ticker-header'),

        dcc.Tabs(id='ticker-tabs', value='prices', className='ticker-tabs', children=[
            dcc.Tab(label='Prices', value='prices', className='ticker-tab', selected_className='ticker-tab--active',
                children=[
                    html.Div(className='price-controls', children=[
                        html.Div(className='price-presets', children=[
                            html.Button(p, id=f'preset-{p}', n_clicks=0, className='btn price-preset-btn')
                            for p in _PRESETS
                        ]),
                        dcc.DatePickerRange(
                            id='price-dates',
                    ...
                    dcc.Loading(dcc.Graph(id='price-chart', config={'displayModeBar': False})),
                    dcc.Loading(html.Div(id='price-table-container')),
                ]),

            dcc.Tab(label='Income', value='income', className='ticker-tab', selected_className='ticker-tab--active',
                children=[
            ...
            dcc.Tab(label='Balance', value='balance', className='ticker-tab', selected_className='ticker-tab--active',
                children=[
            ...
            dcc.Tab(label='Cash Flow', value='cashflow', className='ticker-tab', selected_className='ticker-tab--active',
                children=[
            ...
            dcc.Tab(label='Dividends & Splits', value='actions', className='ticker-tab', selected_className='ticker-tab--active',
                children=[
...
def load_ticker_data(_: Any) -> tuple[Any, ...]:
    ...
    markets = [{'label': m, 'value': m} for m in sorted(uni['Market'].dropna().unique())]
    sectors = [{'label': s, 'value': s} for s in sorted(comp['Sector'].dropna().unique())]
    industries = [{'label': i, 'value': i} for i in sorted(comp['Industry'].dropna().unique())]
...
def render_header(ticker: str | None) -> list:
    ...
    meta_items = [
        ...
        ('CIK', cik),
        ('Currency', currency),
        ('Employees', f'{int(employees):,}' if employees and pd.notna(employees) else ''),
    ]

    return [
        html.Div(className='ticker-title-row', children=[
            html.Span(ticker, className='ticker-symbol'),
            html.Span(name, className='ticker-name') if name else None,
        ]),
        html.Div(className='ticker-badges', children=[
            html.Span(b, className='ticker-badge') for b in badges
        ]) if badges else None,
        html.Div(className='ticker-meta', children=[
            html.Span(className='ticker-meta-item', children=[
                html.Span(f'{label}: ', className='ticker-meta-label'),
                html.Span(val, className='ticker-meta-value'),
            ])
            for label, val in meta_items if val
        ]) if any(v for _, v in meta_items) else None,
    ]
...
def render_prices(
    ...
    try:
        if source == 'stooq':
            ...
            from irp.query.yahoo import prices as _prices
            df = _prices(ticker, start=start, end=end)
            close_col = 'Close'
    ...
    shapes: list[dict] = []
    try:
        from irp.query.yahoo import dividends as _divs, splits as _splits
        ...
        if not divs.empty:
            ...
            price_at_div = df.set_index('_date_str')[close_col].reindex(div_dates).values
    ...
    ohlcv_cols = ['O', 'H', 'L', 'C', 'V'] if source == 'stooq' else ['Open', 'High', 'Low', 'Close', 'Volume']
...
def update_price_table(price_data: list | None, relayout_data: dict | None) -> Any:
    ...
    if triggered == 'price-chart' and relayout_data and 'xaxis.range[0]' in relayout_data:
...
def _render_statement(ticker: str | None, name: str, period: str) -> tuple[Any, str]:
    ...
    rows = [
        html.Tr([
            ...
            *[html.Td(str(fmt_df.loc[item, c]), className='stmt-td') for c in fmt_df.columns],
        ])
        for item in fmt_df.index
    ]
    table = html.Table(className='stmt-table', children=[
        html.Thead(header),
        html.Tbody(rows),
    ])
    return table, note
...
def render_actions(ticker: str | None) -> Any:
    ...
    if divs.empty:
        ...
        div_block = html.Div(children=[
            html.H3('Dividends', className='actions-title'),
            html.Table(className='stmt-table', children=[
                html.Thead(html.Tr([html.Th('Date', className='stmt-th stmt-th--item'), html.Th('Amount', className='stmt-th')])),
                html.Tbody(rows),
            ]),
        ])
    ...
    if not spls.empty:
        ...
        ratio_col = 'Stock Splits' if 'Stock Splits' in spls.columns else spls.columns[-1]
        spl_rows = [
            html.Tr([
                html.Td(str(row['Date'])[:10], className='stmt-td stmt-td--item'),
        ...
        split_block = html.Div(children=[
            html.H3('Splits', className='actions-title'),
            html.Table(className='stmt-table', children=[
                html.Thead(html.Tr([html.Th('Date', className='stmt-th stmt-th--item'), html.Th('Ratio', className='stmt-th')])),
                html.Tbody(spl_rows),
            ]),
        ])
